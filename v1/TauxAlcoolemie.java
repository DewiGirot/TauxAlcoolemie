...
public class TauxAlcoolemie {
	...
	/*
	 * @param : v pour le Volume en millilitre, t pour le taux d'alcoolémie en pourcentage (ex : 0.12 pour 12°) et la masse (en Kg)
	 * @return : retourne la formule pour un homme
	 * */
	public double tauxAlcoolemie(int v, double t) {
		if(getGenre() == SEXE.HOMME) {
			double formule = (v * t * 0.8) / (0.7 * this.masse);
			return formule;
		}else {
			double formule = (v * t * 0.8) / (0.6 * this.masse);
			return formule;
		}
	}
